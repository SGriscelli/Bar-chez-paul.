...
type SortKey = "name" | "brand" | "unitPrice" | "stock" | "floor" | "orderMultiple" | "printQty";
...
function formatCurrency(n?: number) { if (n == null) return "—"; return new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR" }).format(n); }
const PLACEHOLDER =
...
export default function App() {
  ...
  const [invoices, setInvoices] = useState<Invoice[]>(() => load<Invoice[]>("bar_invoices", []));
  ...
  // Option: afficher SKU ?
  const [showSku, setShowSku] = useState<boolean>(() => load("bar_showSku", false));
  ...
  const lowStock = useMemo(() => products.filter(p => p.stock < p.floor), [products]);
  ...
  const sortedProducts = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === "name") {
        ...
        return (sortDir === "asc" ? base : -base) || normalize(a.brand).localeCompare(normalize(b.brand), "fr");
      }
      if (sortBy === "brand") {
        const base = normalize(a.brand).localeCompare(normalize(b.brand), "fr") || normalize(a.name).localeCompare(normalize(b.name), "fr");
        return sortDir === "asc" ? base : -base;
      }
      ...
      if (sortBy === "orderMultiple") return cmpNums(a.orderMultiple, b.orderMultiple);
  ...
  const markRegex = useMemo(() => {
    ...
    if (!t) return null;
    try { return new RegExp(`(${escapeRegExp(t)})`, "gi"); } catch { return null; }
  }, [searchTerm]);
  ...
  const saveProduct = (p: Product) => {
    const normalized: Product = { ...p, printQty: Number.isFinite(p.printQty as number) ? Number(p.printQty) : 0 };
    setProducts(prev => prev.some(x => x.id === p.id) ? prev.map(x => (x.id === p.id ? normalized : x)) : [...prev, normalized]);
    setProductModalOpen(false);
    setEditingProduct(null);
  };

  const deleteProduct = (id: string) => setProducts(prev => prev.filter(p => p.id !== id));
  const inc = (id: string, by = 1) => setProducts(prev => prev.map(p => (p.id === id ? { ...p, stock: Math.max(0, p.stock + by) } : p)));
  const dec = (id: string, by = 1) => setProducts(prev => prev.map(p => (p.id === id ? { ...p, stock: Math.max(0, p.stock - by) } : p)));

  /** ---------- Factures ---------- */
  const openInvoiceEditor = (inv: Invoice) => { setEditingInvoice(inv); setInvoiceModalOpen(true); };
  const saveInvoice = (inv: Invoice) => {
    setInvoices(prev => prev.some(x => x.id === inv.id) ? prev.map(x => (x.id === inv.id ? inv : x)) : [inv, ...prev]);
    setInvoiceModalOpen(false); setEditingInvoice(null);
  };
  const deleteInvoice = (id: string) => setInvoices(prev => prev.filter(i => i.id !== id));
  const imageInputRef = useRef<HTMLInputElement | null>(null);

  const handleImageFileChangeInModal = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const applyInvoiceToStock = (inv: Invoice) => {
    ...
    setProducts(prev => {
      ...
      for (const line of inv.lines) {
        ...
        copy[idx] = { ...copy[idx], stock: Math.max(0, copy[idx].stock + delta) };
      }
      return copy;
    });
  };
  ...
  const generateFromConfiguredQuantities = () => {
    ...
    if (lines.length === 0) { alert("Aucune ligne : mets ‘Qté voulue (facture)’ > 0 pour au moins un produit."); return; }
    const inv: Invoice = {
      id: uid(),
      type: "restock",
...
<script>
  window.onload = () => { window.print(); setTimeout(() => window.close(), 200); };
    ...
    if (!win) { alert("Impossible d’ouvrir la fenêtre d’impression."); return; }
    win.document.open();
  ...
  function printInvoiceCompact(inv: Invoice) {
    ...
    const date = new Date(inv.date).toLocaleString("fr-FR",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});
    const lines = inv.lines.filter(l => l.qty > 0);
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl md:text-3xl font-bold">Gestion de stock — Bar</h1>
          <p className="text-sm text-slate-500">Réassort + seuil plancher + factures imprimables</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            onClick={() =>
              openInvoiceEditor({ id: uid(), type: "blank", date: new Date().toISOString(), lines: [], notes: "", affectsStock: true })
            }
          >
        ...
        <Card>
          <div className="card-header flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-xs text-slate-500">{visibleCount} / {products.length}</span>
              ...
              <label className="text-sm flex items-center gap-2">
                <input type="checkbox" checked={viewGrid} onChange={(e)=>setViewGrid(e.target.checked)} />
              ...
              <Button onClick={openNewProduct}><Plus className="mr-2 h-4 w-4" />Ajouter un produit</Button>
            </div>
          </div>

          <CardContent>
            {products.length === 0 ? (
              <p className="text-sm text-slate-500">Aucun produit. Ajoutez-en via le bouton “Ajouter un produit”.</p>
            ) : visibleCount === 0 ? (
              <div className="rounded-2xl border p-6 text-center text-sm text-slate-500">
                ...
                <button className="underline ml-1" onClick={() => setSearchTerm("")}>Effacer la recherche</button>
              </div>
            ) : viewGrid ? (
              // ====== GRILLE DE CARTES ======
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {sortedProducts.map(p => (
                  <div key={p.id} className="rounded-2xl border overflow-hidden bg-white shadow-sm">
                    ...
                    <div className="p-3 sm:p-4 space-y-3">
                      <div>
                        <div className="text-sm text-slate-500">{highlight(p.brand || "")}</div>
                        <div className="font-semibold">{highlight(p.name)}</div>
                        {showSku && p.sku ? <div className="text-xs text-slate-500">SKU: {highlight(p.sku)}</div> : null}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          <Button size="icon" className="btn-outline" onClick={() => dec(p.id)}><Minus className="h-4 w-4" /></Button>
                          <span className="w-10 text-center">{p.stock}</span>
                          <Button size="icon" className="btn-outline" onClick={() => inc(p.id)}><Plus className="h-4 w-4" /></Button>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="text-right text-sm">{formatCurrency(p.unitPrice)}</div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          value={p.printQty ?? 0}
                          onChange={e =>
                            setProducts(prev => prev.map(x => (x.id === p.id ? { ...x, printQty: Number((e.target as HTMLInputElement).value) } : x)))
                          }
                        />
                        <span className="text-sm text-slate-500">Qté voulue (facture)</span>
                      </div>

                      <div className="flex gap-2 pt-1">
                        <Button className="btn-outline w-full" onClick={() => { setEditingProduct(p); setProductModalOpen(true); }}>
                          Modifier
                        </Button>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">Seuil plancher</TableHead>
                      <TableHead className="text-right">Qté voulue (facture)</TableHead>
                  ...
                  <TableBody>
                    {sortedProducts.map(p => (
                      <TableRow key={p.id} className={p.stock < p.floor ? "bg-red-50/60" : ""}>
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div>
                                {p.brand
                                  ? <span>{highlight(p.brand)} <span className="text-slate-400">•</span> {highlight(p.name)}</span>
                                  : highlight(p.name)
                                }
                              </div>
                              {showSku && p.sku ? <div className="text-xs text-slate-500">SKU: {highlight(p.sku)}</div> : null}
                            </div>
                            {p.stock < p.floor && <Badge variant="destructive">Sous seuil</Badge>}
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Input
                            ...
                            value={p.printQty ?? 0}
                            onChange={e => setProducts(prev => prev.map(x => (x.id === p.id ? { ...x, printQty: Number((e.target as HTMLInputElement).value) } : x)))}
                          />
                        </TableCell>
                        <TableCell className="text-right">{formatCurrency(p.unitPrice)}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button className="btn-outline" onClick={() => { setEditingProduct(p); setProductModalOpen(true); }}>Modifier</Button>
                            <Button variant="destructive" onClick={() => deleteProduct(p.id)}><Trash2 className="h-4 w-4"/></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
          ...
          <div className="px-4 pb-4">
            ...
            >
              ...
              <span className="flex items-center gap-2 text-sm text-slate-600">
                ...
                <ChevronDown className={`h-4 w-4 transition-transform ${showLowPanel ? "rotate-180" : ""}`} />
              </span>
            </button>
                ...
                <CardContent className="space-y-2 p-4">
                  {lowStock.map(p => (
                    <div key={p.id} className="flex items-center justify-between text-sm">
                      <div>{p.brand ? p.brand + " • " : ""}{p.name} <span className="text-slate-500">(stock: {p.stock} / seuil: {p.floor})</span></div>
                      <div className="text-slate-600">Qté configurée: {p.printQty} {p.unit}{p.printQty > 0 ? "(s)" : ""}</div>
                    </div>
                  ))}
                </CardContent>
              </Card>
        ...
        <Card>
          <div className="card-header flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            ...
            <div className="flex gap-2">
              <Button onClick={() => openInvoiceEditor({ id: uid(), type: "sale", date: new Date().toISOString(), lines: [], notes: "", affectsStock: true })}>
                <Plus className="mr-2 h-4 w-4" />Nouvelle facture (vente)
              </Button>
              <Button className="btn-outline" onClick={() => openInvoiceEditor({ id: uid(), type: "blank", date: new Date().toISOString(), lines: [], notes: "", affectsStock: true })}>
                <FileText className="mr-2 h-4 w-4" />Facture vierge (catalogue)
              </Button>
            </div>
          </div>
            ...
            {invoices.length === 0 ? (
              ...
              <div className="rounded-2xl border overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {invoices.map(inv => {
                      const total = inv.lines.reduce((s, l) => s + (l.unitPrice ?? 0) * l.qty, 0);
                      return (
                        <TableRow key={inv.id}>
                          <TableCell>{new Date(inv.date).toLocaleString("fr-FR")}</TableCell>
                          <TableCell>
                            {inv.type === "restock" && <Badge>Réassort</Badge>}
                            {inv.type === "sale" && <Badge variant="secondary">Vente</Badge>}
                            {inv.type === "blank" && <Badge variant="outline">Vierge (catalogue)</Badge>}
                          </TableCell>
                          <TableCell className="text-right">{inv.lines.length}</TableCell>
                          <TableCell className="text-right">{formatCurrency(total)}</TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button className="btn-outline" onClick={() => openInvoiceEditor(inv)}>Ouvrir</Button>
                              <Button variant="destructive" onClick={() => deleteInvoice(inv.id)}><Trash2 className="h-4 w-4" /></Button>
                            </div>
                          </TableCell>
                        </TableRow>
                      );
      ...
      <Dialog open={productModalOpen} onOpenChange={setProductModalOpen}>
        <DialogContent className="max-w-2xl">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e =>
                    setEditingProduct({
                      ...editingProduct,
                      orderMultiple: Number((e.target as HTMLInputElement).value)
                    })
                  }
                />
              </div>
          ...
          <DialogFooter>
            <Button className="btn-outline" onClick={() => setProductModalOpen(false)}>
              Fermer
            </Button>
            {editingProduct && (
      ...
      <Dialog open={invoiceModalOpen} onOpenChange={setInvoiceModalOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              {editingInvoice?.type === "blank" ? "Facture vierge (catalogue seulement)" : editingInvoice?.type === "restock" ? "Commande / Réassort" : "Facture de vente"}
            </DialogTitle>
          </DialogHeader>
          {editingInvoice && (
            <div className="space-y-4 p-4">
              <div className="grid md:grid-cols-3 gap-3">
                <div>
                  <Label>Type</Label>
                  <select className="select" value={editingInvoice.type} onChange={(e)=>setEditingInvoice({ ...editingInvoice, type: e.target.value as any })}>
                ...
                <div>
                  <Label>Date</Label>
                  <input className="input" type="datetime-local" value={new Date(editingInvoice.date).toISOString().slice(0,16)} onChange={(e)=>setEditingInvoice({ ...editingInvoice, date: new Date(e.target.value).toISOString() })}/>
                </div>
                <div className="flex items-end gap-2">
                  <label className="inline-flex items-center gap-2 text-sm text-slate-600">
                    <input type="checkbox" checked={editingInvoice.affectsStock} onChange={(e)=>setEditingInvoice({ ...editingInvoice, affectsStock: e.target.checked })} />
                    Impacte le stock
                  </label>
                </div>
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right" style={{minWidth: 140}}>Qté</TableHead>
                      <TableHead style={{minWidth: 120}}>Unité</TableHead>
                      <TableHead className="text-right" style={{minWidth: 140}}>PU (HT)</TableHead>
                      <TableHead className="text-right" style={{minWidth: 140}}>Total</TableHead>
                      <TableHead className="text-right" style={{minWidth: 120}}>Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {editingInvoice.lines.map((l) => {
                      const selectedProduct = products.find(p => p.id === l.productId);
                      ...
                      return (
                        <TableRow key={l.id}>
                          <TableCell className="min-w-[260px]">
                            {canFreeEdit ? (
                              <Input value={l.label} onChange={e => {
                                const updated = { ...l, label: (e.target as HTMLInputElement).value };
                                setEditingInvoice({ ...editingInvoice, lines: editingInvoice.lines.map(x => x.id === l.id ? updated : x) });
                              }} />
                            ) : (
                              <select
                            ...
                            {(!canFreeEdit && selectedProduct) && (
                              <div className="mt-1 text-xs text-slate-500">
                                Stock actuel: {selectedProduct.stock} • Seuil: {selectedProduct.floor}
                              </div>
                            )}
                          </TableCell>
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-1">
                              <Button size="icon" className="btn-outline" onClick={() => {
                                const updated = { ...l, qty: Math.max(0, l.qty - 1) };
                                setEditingInvoice({ ...editingInvoice, lines: editingInvoice.lines.map(x => x.id === l.id ? updated : x) });
                              }}><Minus className="h-4 w-4"/></Button>
                              <Input type="number" className="w-20 text-right" value={l.qty} onChange={e => {
                                const updated = { ...l, qty: Number((e.target as HTMLInputElement).value) };
                                setEditingInvoice({ ...editingInvoice, lines: editingInvoice.lines.map(x => x.id === l.id ? updated : x) });
                              }} />
                          ...
                          <TableCell className="text-right">
                            <Input type="number" className="w-28 text-right" value={l.unitPrice ?? 0} onChange={e => {
                              const updated = { ...l, unitPrice: Number((e.target as HTMLInputElement).value) || undefined };
                              setEditingInvoice({ ...editingInvoice, lines: editingInvoice.lines.map(x => x.id === l.id ? updated : x) });
                            }} />
                          </TableCell>
                          <TableCell className="text-right">{formatCurrency((l.unitPrice ?? 0) * l.qty)}</TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button variant="destructive" onClick={() => setEditingInvoice({ ...editingInvoice, lines: editingInvoice.lines.filter(x => x.id !== l.id) })}><Trash2 className="h-4 w-4"/></Button>
                            </div>
                          </TableCell>
                        </TableRow>
                      );
                          ...
                          <Button
                            ...
                            onClick={() => setEditingInvoice({
                              ...
                              lines: [
                                ...
                                { id: uid(), label: "", qty: 1, unit: "unité", unitPrice: 0 }
                              ]
                            })}
                          >
              ...
              <div className="grid md:grid-cols-2 gap-3">
                <div>
                  <Label>Notes</Label>
                  <Textarea value={editingInvoice.notes || ""} onChange={e => setEditingInvoice({ ...editingInvoice, notes: (e.target as HTMLTextAreaElement).value })} />
                </div>
                <div className="p-4 rounded-2xl border space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-500">Sous-total (HT)</span>
                    <span className="font-medium">{formatCurrency(editingInvoice.lines.reduce((s, l) => s + (l.unitPrice ?? 0) * l.qty, 0))}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-500">TVA (20% estim.)</span>
                    <span className="font-medium">{formatCurrency(editingInvoice.lines.reduce((s, l) => s + (l.unitPrice ?? 0) * l.qty, 0) * 0.2)}</span>
                  </div>
                  <div className="flex items-center justify-between text-lg">
                    <span className="font-semibold">Total TTC (est.)</span>
                    <span className="font-bold">{formatCurrency(editingInvoice.lines.reduce((s, l) => s + (l.unitPrice ?? 0) * l.qty, 0) * 1.2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 justify-end">
                ...
                <Button className="btn-outline" onClick={() => printInvoiceCompact(editingInvoice!)}>
                  <Printer className="mr-2 h-4 w-4" /> Imprimer (compact 1 page)
                </Button>

                {/* Bouton existant “demande grossiste” */}
                <Button className="btn-outline" onClick={() => printInvoice(editingInvoice!)}>
                  <Printer className="mr-2 h-4 w-4" /> Imprimer (demande grossiste)
                </Button>

                <Button className="btn-outline" onClick={() => saveInvoice(editingInvoice)}>Enregistrer</Button>
                {editingInvoice.affectsStock && (
                  <Button onClick={() => { applyInvoiceToStock(editingInvoice); saveInvoice(editingInvoice); setInvoiceModalOpen(false); }}>
                    Appliquer au stock
                  </Button>
                )}
              </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 p-1">
            <div className="flex items-start gap-3 text-amber-600">
              ...
              <p className="text-sm">
                Cette action remettra <strong>tous les stocks à 0</strong> et <strong>tous les lots de commande à 1</strong>.
          ...
          <DialogFooter>
            <Button className="btn-outline" onClick={() => setResetAllOpen(false)}>Annuler</Button>
            <Button
              variant="destructive"
              onClick={() => {
                setProducts(prev => prev.map(p => ({ ...p, stock: 0, orderMultiple: 1 })));
      ...
      <div className="text-xs text-slate-500 text-center pt-8">
        Astuces: Ctrl/Cmd+P pour imprimer. Données enregistrées localement (localStorage).
      </div>
    </div>
  );
}
